...
useEffect(() => {
  fetch("http://127.0.0.1:8000/venues")
    ...
    .then(data => {
      ...
      const transformed = data.map((v: any, index: number) => ({
        ...
        image: v.image_url || "https://via.placeholder.com/400",
        ...
        happyHour: v.timing || "N/A",

        deal: v.deal || "No deal available",

        address: v.address || "Unknown",

        crowdLevel: "Medium",

        foodType: [v.category || "Food"],
  ...
  // Only filter if the filter actually has a value selected
  if (filters.country && filters.country !== "" && venue.country !== filters.country) return false;
  if (filters.state && filters.state !== "" && venue.state !== filters.state) return false;
  if (filters.city && filters.city !== "" && venue.city !== filters.city) return false;
  
  // For types, ensure the data exists before calling .includes()
  if (filters.venueType && venue.type !== filters.venueType) return false;
  
  if (filters.foodType && (!venue.foodType || !venue.foodType.includes(filters.foodType))) return false;
  if (filters.drinkType && (!venue.drinkTypes || !venue.drinkTypes.includes(filters.drinkType))) return false;
  ...
  console.log("FINAL VENUES:", venues);
//   return (
  
// );
console.log("Filtered Results Count:", filteredVenues.length);
 return (
  ...
  <p className="text-red-500 text-xl">No venues found</p>
  )}
